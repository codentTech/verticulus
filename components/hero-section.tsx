...
export default function HeroSection({ featuredClients }: HeroSectionProps) {
  ...
  const sampleClients = featuredClients.length > 0 ? featuredClients : [
    { name: "TechCorp", logo: "" },
    ...
    >
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0">
          ...
          <svg className="absolute inset-0 w-full h-full opacity-20">
            ...
            <path
              d={`M 0,${mousePosition.y * 4} Q ${mousePosition.x * 8},${mousePosition.y * 2} 800,${mousePosition.y * 6}`}
            ...
            />
            <path
              d={`M ${mousePosition.x * 8},0 Q ${mousePosition.x * 4},400 ${mousePosition.x * 6},800`}
      ...
      <div className="relative z-10 container mx-auto px-6 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center min-h-[80vh]">
          ...
          <div className={`space-y-8 transform transition-all duration-1000 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
            <div className="space-y-6">
              ...
              <p className="text-xl text-gray-300 leading-relaxed max-w-xl">
                No red tape. No gimmicks. Just real outcomes - And yes, AI powers our code
              </p>
            </div>

            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <button className="group relative px-8 py-4 bg-gradient-to-r from-blue-500 to-purple-600 text-white font-semibold rounded-lg overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-2xl">
              ...
              <button className="px-8 py-4 border border-gray-600 text-white font-semibold rounded-lg hover:border-blue-400 hover:shadow-lg transition-all duration-300">
                View Portfolio
              </button>
            </div>
          </div>
          ...
          <div className={`relative flex items-center justify-center transform transition-all duration-1000 delay-300 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="relative">
                  ...
                  <div className="w-72 h-72 rounded-full border-2 border-gradient animate-spin-slow">
                    ...
                    <div className="absolute bottom-0 left-1/2 w-3 h-3 bg-purple-400 rounded-full transform -translate-x-1/2 translate-y-1/2 animate-pulse" style={{ animationDelay: '0.5s' }}></div>
                    <div className="absolute left-0 top-1/2 w-2 h-2 bg-pink-400 rounded-full transform -translate-x-1/2 -translate-y-1/2 animate-pulse" style={{ animationDelay: '1s' }}></div>
                    <div className="absolute right-0 top-1/2 w-3 h-3 bg-cyan-400 rounded-full transform translate-x-1/2 -translate-y-1/2 animate-pulse" style={{ animationDelay: '1.5s' }}></div>
                  </div>
      ...
      <style jsx>{`
        ...
        @keyframes morph {
          ...
          50% { border-radius: 80% 20% 50% 80% / 55% 40% 60% 45%; }
          75% { border-radius: 40% 70% 30% 90% / 60% 50% 40% 50%; }
        }

        @keyframes morph-reverse {
          0%, 100% { border-radius: 30% 70% 70% 30% / 30% 30% 70% 70%; }
          25% { border-radius: 70% 30% 50% 60% / 60% 50% 30% 40%; }
          50% { border-radius: 20% 80% 40% 60% / 45% 60% 55% 40%; }
          75% { border-radius: 60% 40% 80% 30% / 50% 40% 60% 50%; }
        }
        ...
        .border-gradient {
          border-image: linear-gradient(45deg, rgba(59, 130, 246, 0.5), rgba(147, 51, 234, 0.5), rgba(236, 72, 153, 0.5)) 1;
        }
      `}</style>
    </section>
  )
